...
function rows(orders: Order[]) {
  return orders.map((order) => [
    ...
    Number(order.settledFromStaff).toFixed(2),
    Number(order.settledFromDrawer).toFixed(2),
...
export async function exportOrdersPdf(
  ...
  autoTable(document, {
    ...
    body: orders.map((order) => [
      ...
      order.customerName,
      order.paymentMethod,
      order.paymentMethod === "card" ? (order.cardAccount ?? "").toUpperCase() : order.assignedStaffName,
      order.paymentMethod === "cash" ? `${Number(order.cashReceived).toFixed(2)} / ${Number(order.settledFromStaff).toFixed(2)} / ${Number(order.settledFromDrawer).toFixed(2)}` : "",
      order.paymentStatus,
      Number(order.vatAmount).toFixed(2),
      Number(order.totalAmount).toFixed(2),
      Number(order.amountPaid).toFixed(2),
