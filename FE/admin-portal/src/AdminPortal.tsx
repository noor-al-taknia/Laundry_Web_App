...
function AdminLogin({ onLogin, error, locale, changeLocale }: { onLogin: (user: User) => Promise<void>; error: string; locale: Locale; changeLocale: (locale: Locale) => void }) {
  ...
  return <main className="admin-login" dir={locale === "ar" ? "rtl" : "ltr"}><form onSubmit={submit}><div className="login-language"><button type="button" onClick={() => changeLocale(locale === "en" ? "ar" : "en")}>{locale === "en" ? "العربية" : "English"}</button></div><div className="admin-login-mark">PL</div><p>{tr(locale, "ADMIN PORTAL", "بوابة الإدارة")}</p><h1>{tr(locale, "Manage your laundry business", "إدارة أعمال المغسلة بسهولة")}</h1><span>{tr(locale, "Owners and authorized technical administrators only.", "للمالك ومسؤولي التقنية المصرح لهم فقط.")}</span>{message && <div>{message}</div>}<label>{tr(locale, "Username", "اسم المستخدم")}<input value={username} onChange={(event) => setUsername(event.target.value)} required /></label><label>{tr(locale, "Password", "كلمة المرور")}<span className="password-field"><input type={showPassword ? "text" : "password"} value={password} onChange={(event) => setPassword(event.target.value)} required /><button type="button" onClick={() => setShowPassword((shown) => !shown)}>{showPassword ? "◉" : "◌"}</button></span></label><button disabled={busy}>{busy ? tr(locale, "Signing in…", "جارٍ تسجيل الدخول…") : tr(locale, "Sign in", "تسجيل الدخول")}</button><div className="login-help"><button type="button" onClick={() => setMessage(tr(locale,"Admins reset passwords through another authorized admin or super-admin.","يتم تغيير كلمة مرور المسؤول بواسطة مسؤول آخر أو المسؤول التقني."))}>{tr(locale,"Reset password","إعادة كلمة المرور")}</button><button type="button" onClick={() => setMessage(tr(locale,"Contact your super-admin if you cannot sign in.","تواصل مع المسؤول التقني إذا تعذر تسجيل الدخول."))}>{tr(locale,"Forgot password?","نسيت كلمة المرور؟")}</button></div><Link href="/">{tr(locale, "Return to office portal", "العودة إلى بوابة المكتب")}</Link></form></main>;
}
