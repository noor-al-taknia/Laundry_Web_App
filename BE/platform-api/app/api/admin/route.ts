...
export async function POST(request: Request) {
  return route(async () => {
    ...
    switch (body.action) {
      ...
      case "user.create": {
        ...
        await db
          .prepare(
            `INSERT INTO users
             (username, display_name, role, portal_role, password_hash, password_salt,
              phone, passport_number, passport_expiry, visa_status, visa_expiry,
          ...
          .bind(
            ...
            passwordData.salt,
            textValue(body.phone, 40),
            textValue(body.passportNumber, 60),
            textValue(body.passportExpiry, 10) || null,
            textValue(body.visaStatus, 40) || "not_recorded",
            textValue(body.visaExpiry, 10) || null,
            textValue(body.iqamaNumber, 60),
            textValue(body.iqamaExpiry, 10) || null,
          )
          .run();
        return { ok: true };
      }
          ...
          db
            .prepare(
              `UPDATE users SET display_name = ?, role = ?, portal_role = ?, is_active = ?,
               phone = ?, passport_number = ?, passport_expiry = ?, visa_status = ?,
               visa_expiry = ?, iqama_number = ?, iqama_expiry = ?,
               updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
            )
            .bind(
      ...
      case "staff_debt.update": {
        const id = Number(body.id);
        const debt = await db.prepare(
          "SELECT original_amount AS originalAmount FROM staff_debts WHERE id = ?",
        ).bind(id).first<{ originalAmount: number }>();
        if (!debt) return json({ error: "Staff debt record not found." }, 404);
        const outstandingAmount = Math.max(0, Math.min(Number(body.outstandingAmount ?? 0), Number(debt.originalAmount)));
        const status = outstandingAmount === 0 ? "settled" : "open";
        await db.prepare(
          `UPDATE staff_debts SET outstanding_amount = ?, status = ?, notes = ?,
           settled_by = CASE WHEN ? = 'settled' THEN ? ELSE NULL END,
           settled_at = CASE WHEN ? = 'settled' THEN CURRENT_TIMESTAMP ELSE NULL END,
           updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
        ).bind(outstandingAmount, status, textValue(body.notes, 500), status, admin.id, status, id).run();
        return { ok: true };
      }
      case "grant.create": {
